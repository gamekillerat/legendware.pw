class LiteYTEmbed extends HTMLElement{constructor(){super();this.__iframeLoaded=false;this.__setupDom();}
static get observedAttributes(){return['videoid'];}
connectedCallback(){this.addEventListener('pointerover',LiteYTEmbed.warmConnections,{once:true,});this.addEventListener('click',e=>this.__addIframe());}
...
__setupComponent(){this.videoId=encodeURIComponent(this.getAttribute('videoid'));this.videoTitle=this.getAttribute('videotitle')||'Video';this.videoPlay=this.getAttribute('videoplay')||'Play';this.videoStartAt=this.getAttribute('start')||0;this.autoLoad=this.getAttribute('autoload')===''?true:false;this.__initImagePlaceholder();this.__domRefPlayButton.setAttribute('aria-label',`${this.videoPlay}: ${this.videoTitle}`,);this.setAttribute('title',`${this.videoPlay}: ${this.videoTitle}`);if(this.autoLoad){this.__initIntersectionObserver();}}
...
></iframe>`;this.__domRefFrame.insertAdjacentHTML('beforeend',iframeHTML);this.__domRefFrame.classList.add('lyt-activated');this.__iframeLoaded=true;}}
...
static warmConnections(){if(LiteYTEmbed.preconnected)return;LiteYTEmbed.addPrefetch('preconnect','https://s.ytimg.com');LiteYTEmbed.addPrefetch('preconnect','https://www.youtube.com');LiteYTEmbed.addPrefetch('preconnect','https://www.google.com');LiteYTEmbed.addPrefetch('preconnect','https://googleads.g.doubleclick.net',);LiteYTEmbed.addPrefetch('preconnect','https://static.doubleclick.net');LiteYTEmbed.preconnected=true;}}
